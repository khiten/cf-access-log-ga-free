# cf-access-log-ga-free (PHP版)

PHP CLIのみで動作する実装です。追加パッケージのインストールは不要です（curl・zlib・json拡張は多くの共用サーバーで標準有効）。

全体の説明・できないこと・出力仕様は[リポジトリ全体のREADME](../README.md)を参照してください。ここではPHP版固有のセットアップ手順のみ説明します。

## セットアップ

1. `php`ディレクトリ一式をサーバーに設置します。出力にはアクセス元IP・User-Agent等が含まれるため、**Webから直接アクセスできない場所**（`public_html`等の公開ディレクトリの外）に置いてください
...
6. cronに登録します（10分ごとの例）
   ```
   */10 * * * * /path/to/cf-access-log-ga-free/php/run.sh >> /path/to/cf-access-log-ga-free/php/run.log 2>&1
   ```
...
- bash（`run.sh`の実行）
- PHP CLI 7.4以上
- PHP拡張: `curl`・`zlib`・`json`（多くの共用サーバーで標準有効。起動時に自動チェックされ、不足時は分かりやすいエラーで終了します）
...
- **`PHP拡張 '...' が有効になっていません`**: サーバー管理者・レンタルサーバーの管理画面でPHP拡張の有効化状況を確認してください（curl・zlib・jsonが必要です）
...
- **`cannot request data older than ...`というGraphQLエラー**: Cloudflare側の取得可能期間の上限（実測で約1週間+1日）を超えた古い日付を要求しています。`TARGET_DAYS_AGO`の設定ミス、またはcronが長期間停止していて未処理の日付が溜まっている可能性があります。この上限を超えた日付のデータはCloudflare側にも残っておらず、**復旧できません**。まずcronが正常に動き続けているか（`summary.json`の生成日時が毎日更新されているか）を確認してください
